...
def train_router():
    ...
    # Compute scale_pos_weight for balanced classes
    ...
    # Rest are layers
    print(f"Layer Cosine Sims (First 5): {importances[:5]}")
    print(f"Layer Cosine Sims (Last 5): {importances[-6:-1]}")

    # Save to file
    output_dict = {
        "metrics": {
            "best_threshold": float(best_thresh),
            "f1_score": float(best_f1) if best_f1 > 0 else 0.0,
        ...
        "feature_importances": {
            ...
            "cosine_sims_importance": [float(i) for i in importances[:-1]]
        }
    }
    
    with open('router_xgb_results.json', 'w') as f:
        json.dump(output_dict, f, indent=4)
        
    print("\nDone! Results and importances saved to router_xgb_results.json")
