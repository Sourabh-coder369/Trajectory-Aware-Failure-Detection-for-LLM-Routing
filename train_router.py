...
def train_router():
    ...
    coefs = model.coef_[0]
    # Last one is Entropy
    print(f"Final Entropy Coef: {coefs[-1]:.4f}")
    # Rest are layers
    print(f"Layer Cosine Sims (First 5): {coefs[:5]}")
    print(f"Layer Cosine Sims (Last 5): {coefs[-6:-1]}")
    ...
    with open('router_results.json', 'w') as f:
        json.dump(output_dict, f, indent=4)
        
    print("\nDone! Results and coefficients saved to router_results.json")
